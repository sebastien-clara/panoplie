...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "dryWet");
		m->declare("name", "dryWet");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("dryWet");
		ui_interface->declare(&fHslider0, "1", "");
		ui_interface->declare(&fHslider0, "style", "knob");
		ui_interface->addHorizontalSlider("Dry-Wet", &fHslider0, 0.0, -1.0, 1.0, 0.10000000000000001);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		double fSlow0 = (0.5 * (double(fHslider0) + -1.0));
		double fSlow1 = std::sqrt((0.0 - fSlow0));
		double fSlow2 = std::sqrt((fSlow0 + 1.0));
		for (int i = 0; (i < count); i = (i + 1)) {
			double fTemp0 = (fSlow1 * double(input0[i]));
			double fTemp1 = (fSlow2 * double(input1[i]));
			output0[i] = FAUSTFLOAT((fTemp0 + fTemp1));
